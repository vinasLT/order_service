...
async def set_custom_invoice_added(
    ...
    if order.delivery_status != OrderStatusEnum.VEHICLE_IN_CUSTOM_AGENCY:
        raise BadRequestProblem(
            detail=f"You cannot change status because it is not {OrderStatusEnum.VEHICLE_IN_CUSTOM_AGENCY.value}"
        )
