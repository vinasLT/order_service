...
async def make_invoice_visible_invoice_visible(
    ...
    if order.delivery_status != OrderStatusEnum.PORT_CHOSEN:
        raise BadRequestProblem(detail=f"You cannot change status because it is not {OrderStatusEnum.PORT_CHOSEN.value}")

    await order_service.update(order_id, OrderUpdate(delivery_status=OrderStatusEnum.INVOICE_ADDED))
